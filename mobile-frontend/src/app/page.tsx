  ...
  const handleClick = async () => {
    startTransition(async () => {
      ...
      setOptimisticStats((currentStats) => ({
        ...
        recentAccess: [{ accessed_at: new Date().toISOString() }, ...currentStats.recentAccess.slice(0, 4)]
      }))
      try {
        // Actual update
        ...
        <Card className="w-full max-w-md shadow-xl dark:bg-gray-900/80 backdrop-blur-sm border-gray-200 dark:border-gray-700/50">
          ...
          <CardContent className="flex flex-col items-center gap-6 p-6">
            ...
            <div className="w-full pt-4">
              <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 text-center">Recent Activity</h3>
              <ScrollArea className="h-[120px] w-full rounded-md border border-gray-200 dark:border-gray-700/50 p-3 bg-gray-50/50 dark:bg-gray-800/30">
                <AnimatePresence initial={false}>
                  {optimisticStats.recentAccess.length > 0 ? (
                    ...
                    <div className="text-xs text-gray-400 dark:text-gray-500 text-center py-4">No recent activity</div>
                  )}
                </AnimatePresence>
              </ScrollArea>
            </div>
