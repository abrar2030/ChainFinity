...
interface AppActions {
  ...
  register: (userData: any) => Promise<{ success: boolean; data?: any; error?: ApiError }>; // Replace 'any' with specific user data type
  logout: () => void;
  clearError: () => void;
  toggleTheme: () => void;
...
const AppContext = createContext<AppState & { actions: AppActions } | undefined>(undefined);
...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  const login = useCallback(async (credentials: any): Promise<boolean> => {
    ...
    try {
      ...
      setState(prevState => ({ ...prevState, error: apiError, loading: false }));
      return false;
    }
  }, []);

  // Register function
  const register = useCallback(async (userData: any): Promise<{ success: boolean; data?: any; error?: ApiError }> => {
    setState(prevState => ({ ...prevState, loading: true, error: null }));
    try {
  ...
  const connectWallet = useCallback(async () => {
    if (typeof window.ethereum !== 'undefined') {
      try {
        ...
        setState(prevState => ({ ...prevState, error: { message: 'Failed to connect wallet. Please try again.', status: 500 } }));
      }
    } else {
      console.error('MetaMask (or other Ethereum wallet) not detected.');
      setState(prevState => ({ ...prevState, error: { message: 'Wallet not detected. Please install MetaMask.', status: 404 } }));
