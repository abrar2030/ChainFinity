...
class ComplianceCheck(BaseModel, TimestampMixin, AuditMixin):
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            (self.valid_until is None or self.valid_until > datetime.utcnow()) and
            self.status in [ComplianceStatus.PASSED, ComplianceStatus.EXEMPTED]
        )
...
class SuspiciousActivityReport(BaseModel, TimestampMixin, AuditMixin):
    ...
    def is_filing_overdue(self) -> bool:
        ...
        return (
            self.filing_required and
            not self.filed_at and
            self.filing_deadline and
            self.filing_deadline < datetime.utcnow()
        )
