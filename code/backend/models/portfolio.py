...
from sqlalchemy import (
    ...
    ForeignKey, Integer, Numeric, JSON, Index, UniqueConstraint
)
...
class Portfolio(BaseModel, TimestampMixin, AuditMixin):
    ...
    portfolio_type = Column(Enum(PortfolioType), default=PortfolioType.MAIN, nullable=False, index=True)
    ...
    rebalance_frequency = Column(Enum(RebalanceFrequency), default=RebalanceFrequency.MANUAL, nullable=False)
    
    # Allocation Strategy
    allocation_strategy = Column(Enum(AllocationStrategy), default=AllocationStrategy.CUSTOM, nullable=False)
...
class PortfolioAsset(BaseModel, TimestampMixin, AuditMixin):
    ...
    asset_type = Column(Enum(AssetType), default=AssetType.CRYPTOCURRENCY, nullable=False, index=True)
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint('portfolio_id', 'asset_symbol', 'contract_address', name='uq_portfolio_asset'),
...
class AssetAllocation(BaseModel, TimestampMixin, AuditMixin):
    ...
    # Rebalancing
    rebalance_threshold = Column(Numeric(5, 2), default=5.0, nullable=False)  # Deviation threshold
    is_active = Column(Boolean, default=True, nullable=False)
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint('portfolio_id', 'asset_symbol', name='uq_portfolio_allocation'),
    )
...
class PortfolioPerformance(BaseModel, TimestampMixin):
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint('portfolio_id', 'period_start', 'period_end', 'period_type', name='uq_portfolio_performance'),
