...
class RedisSettings(BaseSettings):
    ...
    SESSION_TTL: int = Field(default=86400, env="SESSION_TTL")
...
class BlockchainSettings(BaseSettings):
    ...
    ETH_RPC_URL: str = Field(
        default="https://mainnet.infura.io/v3/YOUR_PROJECT_ID", env="ETH_RPC_URL"
    )
    ...
    GOVERNANCE_TOKEN_ADDRESS: Optional[str] = Field(
        default=None, env="GOVERNANCE_TOKEN_ADDRESS"
    )
...
class AppSettings(BaseSettings):
    ...
    APP_DESCRIPTION: str = Field(
        default="Production-ready DeFi Analytics Platform", env="APP_DESCRIPTION"
    )
