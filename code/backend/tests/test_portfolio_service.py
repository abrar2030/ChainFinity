...
from schemas.portfolio import PortfolioCreate, PortfolioUpdate, AssetAllocation
...
class TestPortfolioService:
    ...
    @pytest.mark.asyncio
    async def test_create_portfolio_success(self, portfolio_service, sample_user, db_session):
        ...
        result = await portfolio_service.create_portfolio(sample_user.id, portfolio_data)
    ...
    @pytest.mark.asyncio
    async def test_create_portfolio_invalid_cash_amount(self, portfolio_service, sample_user):
    ...
    @pytest.mark.asyncio
    async def test_create_portfolio_duplicate_name(self, portfolio_service, sample_user, db_session):
        ...
        db_session.execute = AsyncMock(return_value=Mock(scalar_one_or_none=Mock(return_value=existing_portfolio)))
        
        with pytest.raises(ValueError, match="Portfolio with this name already exists"):
            await portfolio_service.create_portfolio(sample_user.id, portfolio_data)
    
    @pytest.mark.asyncio
    async def test_create_portfolio_exceeds_limit(self, portfolio_service, sample_user, db_session):
        ...
        db_session.execute = AsyncMock(return_value=Mock(scalar=Mock(return_value=10)))  # Max limit
    ...
    @pytest.mark.asyncio
    async def test_get_portfolio_success(self, portfolio_service, sample_portfolio, db_session):
        """Test successful portfolio retrieval"""
        db_session.execute = AsyncMock(return_value=Mock(scalar_one_or_none=Mock(return_value=sample_portfolio)))
        
        result = await portfolio_service.get_portfolio(sample_portfolio.id, sample_portfolio.user_id)
    ...
    @pytest.mark.asyncio
    async def test_get_user_portfolios(self, portfolio_service, sample_user, sample_portfolio, db_session):
        """Test retrieving all portfolios for a user"""
        portfolios = [sample_portfolio]
        db_session.execute = AsyncMock(return_value=Mock(scalars=Mock(return_value=Mock(all=Mock(return_value=portfolios)))))
    ...
    @pytest.mark.asyncio
    async def test_update_portfolio_success(self, portfolio_service, sample_portfolio, db_session):
    ...
    @pytest.mark.asyncio
    async def test_add_asset_success(self, portfolio_service, sample_portfolio, db_session):
        ...
        with patch.object(portfolio_service, '_get_current_price', return_value=Decimal("3200.00")):
    ...
    @pytest.mark.asyncio
    async def test_add_asset_insufficient_cash(self, portfolio_service, sample_portfolio, db_session):
    ...
    @pytest.mark.asyncio
    async def test_remove_asset_success(self, portfolio_service, sample_portfolio, sample_asset, db_session):
        ...
        with patch.object(portfolio_service, '_get_current_price', return_value=Decimal("50000.00")):
    ...
    @pytest.mark.asyncio
    async def test_update_asset_quantity(self, portfolio_service, sample_portfolio, sample_asset, db_session):
    ...
    @pytest.mark.asyncio
    async def test_calculate_portfolio_value(self, portfolio_service, sample_portfolio, sample_asset, db_session):
        ...
        expected_value = sample_portfolio.cash_balance + (sample_asset.quantity * Decimal("50000.00"))
        assert total_value == expected_value
    
    @pytest.mark.asyncio
    async def test_get_portfolio_performance(self, portfolio_service, sample_portfolio, db_session):
    ...
    @pytest.mark.asyncio
    async def test_rebalance_portfolio_success(self, portfolio_service, sample_portfolio, db_session):
        ...
        with patch.object(portfolio_service, '_execute_rebalancing_trades') as mock_trades:
    ...
    @pytest.mark.asyncio
    async def test_rebalance_invalid_allocation(self, portfolio_service, sample_portfolio):
        """Test rebalancing with invalid allocation percentages"""
        target_allocations = [
            AssetAllocation(symbol="BTC", target_percentage=Decimal("60.0")),
            AssetAllocation(symbol="ETH", target_percentage=Decimal("50.0"))  # Total > 100%
        ]
        
        with pytest.raises(InvalidAllocationError, match="Total allocation exceeds 100%"):
    ...
    @pytest.mark.asyncio
    async def test_calculate_portfolio_risk(self, portfolio_service, sample_portfolio, db_session):
    ...
    @pytest.mark.asyncio
    async def test_check_risk_limits(self, portfolio_service, sample_portfolio, db_session):
    ...
    @pytest.mark.asyncio
    async def test_get_transaction_history(self, portfolio_service, sample_portfolio, db_session):
        """Test retrieving transaction history"""
        mock_transactions = [Mock(), Mock(), Mock()]
        db_session.execute = AsyncMock(return_value=Mock(scalars=Mock(return_value=Mock(all=Mock(return_value=mock_transactions)))))
    ...
    @pytest.mark.asyncio
    async def test_generate_portfolio_report(self, portfolio_service, sample_portfolio, db_session):
        """Test portfolio report generation"""
        db_session.execute = AsyncMock(return_value=Mock(scalar_one_or_none=Mock(return_value=sample_portfolio)))
        
        with patch.object(portfolio_service, '_compile_portfolio_analytics') as mock_analytics:
    ...
    async def test_database_error_handling(self, portfolio_service, db_session):
        ...
        db_session.execute = AsyncMock(side_effect=Exception("Database connection error"))
    ...
    @pytest.mark.asyncio
    async def test_concurrent_portfolio_updates(self, portfolio_service, sample_portfolio, db_session):
    ...
    @pytest.mark.asyncio
    async def test_full_portfolio_lifecycle(self, portfolio_service, sample_user, db_session):
        ...
        portfolio = await portfolio_service.create_portfolio(sample_user.id, portfolio_data)
        assert portfolio.name == portfolio_data.name
        
        # Mock portfolio retrieval for subsequent operations
        db_session.execute = AsyncMock(return_value=Mock(scalar_one_or_none=Mock(return_value=portfolio)))
    ...
    @pytest.mark.asyncio
    async def test_large_portfolio_performance(self, portfolio_service, sample_portfolio, db_session):
        ...
        with patch.object(portfolio_service, '_get_current_price', return_value=Decimal("100.0")):
    ...
    @pytest.mark.asyncio
    async def test_zero_quantity_asset(self, portfolio_service, sample_portfolio, db_session):
    ...
    @pytest.mark.asyncio
    async def test_negative_price_asset(self, portfolio_service, sample_portfolio, db_session):
    ...
    @pytest.mark.asyncio
    async def test_empty_portfolio_calculations(self, portfolio_service, sample_portfolio, db_session):
