...
from models.user import User, UserKYC, UserRiskProfile, KYCStatus, RiskLevel
from models.transaction import Transaction, TransactionStatus, TransactionType
from models.compliance import ComplianceCheck, SuspiciousActivity, RegulatoryReport
...
class TestComplianceService:
    ...
    async def test_kyc_verification_success(self, compliance_service: ComplianceService, test_user: User):
        ...
        with patch.object(compliance_service, '_verify_identity') as mock_identity, \
             patch.object(compliance_service, '_verify_documents') as mock_documents, \
             patch.object(compliance_service, '_screen_sanctions') as mock_sanctions, \
             patch.object(compliance_service, '_screen_pep') as mock_pep:
            ...
            result = await compliance_service.perform_kyc_check(str(test_user.id), kyc_data)
            
            assert result.status == "passed"
            assert result.score >= 80.0
            assert result.findings is not None
    
    async def test_kyc_verification_failure(self, compliance_service: ComplianceService, test_user: User):
    ...
    async def test_transaction_monitoring_normal(self, compliance_service: ComplianceService, 
                                               test_transaction: Transaction):
        """Test transaction monitoring for normal transaction"""
        with patch.object(compliance_service, '_check_transaction_amount') as mock_amount, \
             patch.object(compliance_service, '_check_transaction_velocity') as mock_velocity, \
             patch.object(compliance_service, '_check_address_risk') as mock_address, \
             patch.object(compliance_service, '_check_transaction_patterns') as mock_patterns:
            ...
            result = await compliance_service.monitor_transaction(str(test_transaction.id))
            
            assert result.status == "passed"
            assert result.score <= 30.0  # Low risk
            assert result.risk_level in ["low", "medium"]
    
    async def test_transaction_monitoring_suspicious(self, compliance_service: ComplianceService,
                                                   test_transaction: Transaction):
        ...
        with patch.object(compliance_service, '_check_transaction_amount') as mock_amount, \
             ...
             patch.object(compliance_service, '_check_address_risk') as mock_address, \
             patch.object(compliance_service, '_check_transaction_patterns') as mock_patterns, \
             patch.object(compliance_service, '_create_suspicious_activity') as mock_sar:
    ...
    async def test_velocity_check(self, compliance_service: ComplianceService, 
    ...
    async def test_address_risk_assessment(self, compliance_service: ComplianceService):
        ...
        with patch.object(compliance_service, '_query_aml_provider') as mock_aml:
            ...
            assert result["risk_score"] > 0
            assert "from_address_risk" in result["findings"] or "to_address_risk" in result["findings"]
    ...
    async def test_regulatory_report_generation(self, compliance_service: ComplianceService,
    ...
    async def test_compliance_score_calculation(self, compliance_service: ComplianceService):
    ...
    async def test_risk_level_determination(self, compliance_service: ComplianceService):
    ...
    async def test_compliance_status_determination(self, compliance_service: ComplianceService):
...
class TestKYCService:
    ...
    async def test_kyc_submission(self, kyc_service: KYCService, test_user: User):
        ...
        with patch.object(kyc_service, '_validate_kyc_data') as mock_validate, \
             patch.object(kyc_service, '_submit_to_provider') as mock_submit:
            ...
            result = await kyc_service.submit_kyc_verification(str(test_user.id), kyc_data)
...
class TestRiskService:
    ...
    async def test_user_risk_assessment(self, risk_service: RiskService, test_user: User):
