...
class MarketDataService:
    ...
    def __init__(self):
        ...
        self.cache_ttl = {
            ...
            "technical_indicators": 120  # 2 minutes
        }
    ...
    async def get_current_price(self, symbol: str) -> Optional[Decimal]:
        ...
        try:
            ...
            if price:
                # Cache the price
                await cache.set(cache_key, str(price), ttl=self.cache_ttl["current_price"])
                return price
        ...
        try:
            ...
            if cached_data:
                ...
                return MarketData(
                    ...
                    volume_24h=Decimal(data["volume_24h"]) if data["volume_24h"] else None,
                    change_24h=Decimal(data["change_24h"]) if data["change_24h"] else None,
                    change_24h_pct=Decimal(data["change_24h_pct"]) if data["change_24h_pct"] else None,
                    market_cap=Decimal(data["market_cap"]) if data["market_cap"] else None,
            ...
            if market_data:
                ...
                cache_data = {
                    ...
                    "volume_24h": str(market_data.volume_24h) if market_data.volume_24h else None,
                    "change_24h": str(market_data.change_24h) if market_data.change_24h else None,
                    "change_24h_pct": str(market_data.change_24h_pct) if market_data.change_24h_pct else None,
                    "market_cap": str(market_data.market_cap) if market_data.market_cap else None,
                    "timestamp": market_data.timestamp.isoformat(),
                    "source": market_data.source
                }
                
                await cache.set(cache_key, json.dumps(cache_data), ttl=self.cache_ttl["market_stats"])
                return market_data
    ...
    async def get_historical_data(
        ...
        try:
            ...
            # Fetch from external API
            historical_data = await self._fetch_historical_data(symbol, start_date, end_date, interval)
            ...
            if historical_data:
                ...
                await cache.set(cache_key, json.dumps(cache_data), ttl=self.cache_ttl["historical_data"])
        ...
        try:
            ...
            if cached_data:
                ...
                return TechnicalIndicators(
                    ...
                    macd_signal=Decimal(data["macd_signal"]) if data["macd_signal"] else None,
                    bollinger_upper=Decimal(data["bollinger_upper"]) if data["bollinger_upper"] else None,
                    bollinger_lower=Decimal(data["bollinger_lower"]) if data["bollinger_lower"] else None,
                    volatility=Decimal(data["volatility"]) if data["volatility"] else None
                )
            ...
            if indicators:
                ...
                cache_data = {
                    ...
                    "macd_signal": str(indicators.macd_signal) if indicators.macd_signal else None,
                    "bollinger_upper": str(indicators.bollinger_upper) if indicators.bollinger_upper else None,
                    "bollinger_lower": str(indicators.bollinger_lower) if indicators.bollinger_lower else None,
                    "volatility": str(indicators.volatility) if indicators.volatility else None
                }
                
                await cache.set(cache_key, json.dumps(cache_data), ttl=self.cache_ttl["technical_indicators"])
                return indicators
    ...
    async def validate_symbol(self, symbol: str) -> bool:
        ...
        """
        try:
            # Check if we can get a price for the symbol
    ...
    async def get_market_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            market_data_tasks = [self.get_market_data(symbol) for symbol in major_symbols]
    ...
    async def _fetch_historical_data(
        ...
        try:
            # This would integrate with actual historical data APIs
            # For now, return mock data
            data = []
    ...
    async def _calculate_technical_indicators(self, symbol: str) -> Optional[TechnicalIndicators]:
        ...
        try:
            ...
            start_date = end_date - timedelta(days=250)  # Need enough data for 200-day SMA
            ...
            indicators = TechnicalIndicators(
                ...
                macd=None,  # Will be calculated from EMAs
                macd_signal=None,
    ...
    def _calculate_volatility(self, prices: List[Decimal], period: int) -> Optional[Decimal]:
    ...
    def _calculate_market_sentiment(self, gainers: List[Dict], losers: List[Dict]) -> str:
