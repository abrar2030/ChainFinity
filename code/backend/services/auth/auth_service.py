...
class AuthService:
    ...
    async def authenticate_user(
        ...
        try:
            ...
            if not user:
                await self._log_failed_login(db, email, ip_address, user_agent, "user_not_found")
                raise HTTPException(
            ...
            if user.is_locked():
                await self._log_failed_login(db, email, ip_address, user_agent, "account_locked")
                raise HTTPException(
            ...
            if not user.can_login():
                await self._log_failed_login(db, email, ip_address, user_agent, "account_inactive")
                raise HTTPException(
            ...
            if not self.password_service.verify_password(password, user.hashed_password):
                ...
                await db.commit()
                await self._log_failed_login(db, email, ip_address, user_agent, "invalid_password")
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Invalid credentials"
                )
            ...
            if user.mfa_enabled:
                ...
                if not self.mfa_service.verify_totp(user.mfa_secret, mfa_code):
                    user.increment_failed_login()
                    await db.commit()
                    await self._log_failed_login(db, email, ip_address, user_agent, "invalid_mfa")
                    raise HTTPException(
                        status_code=status.HTTP_401_UNAUTHORIZED,
