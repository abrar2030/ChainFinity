        ...
        try:
            ...
            for source in self.sources:
                try:
                    ...
                    logger.warning(
                        f"Error getting price from {source.__class__.__name__}: {e}"
                    )
                    continue
            ...
            if len(prices) % 2 == 0:
                median_price = (
                    prices[len(prices) // 2 - 1] + prices[len(prices) // 2]
                ) / 2
            else:
...
class PriceValidator:
    ...
    @staticmethod
    def validate_price(
        symbol: str, price: Decimal, historical_prices: List[Decimal]
    ) -> bool:
        """
