...
def upgrade():
    ...
    op.create_table('audit_logs',
        ...
        sa.Column('severity', sa.Enum('low', 'medium', 'high', 'critical', name='severitylevel'), nullable=False, default='low'),
    ...
    op.create_table('compliance_checks',
        ...
        sa.Column('check_type', sa.Enum('kyc', 'aml', 'sanctions', 'pep', 'transaction_monitoring', 'risk_assessment', name='compliancechecktype'), nullable=False),
        sa.Column('status', sa.Enum('pending', 'in_progress', 'passed', 'failed', 'manual_review', name='compliancecheckstatus'), nullable=False, default='pending'),
    ...
    op.create_index('idx_compliance_check_transaction_id', 'compliance_checks', ['transaction_id'])
    ...
    op.create_index('idx_compliance_check_risk_level', 'compliance_checks', ['risk_level'])
    ...
    op.create_table('suspicious_activities',
        ...
        sa.Column('transaction_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('transactions.id'), nullable=True),
        sa.Column('activity_type', sa.Enum('unusual_volume', 'rapid_transactions', 'high_risk_address', 'sanctions_match', 'pattern_anomaly', 'velocity_check', name='suspiciousactivitytype'), nullable=False),
        sa.Column('severity', sa.Enum('low', 'medium', 'high', 'critical', name='severitylevel'), nullable=False),
        sa.Column('status', sa.Enum('open', 'investigating', 'resolved', 'false_positive', 'escalated', name='suspiciousactivitystatus'), nullable=False, default='open'),
    ...
    op.create_index('idx_suspicious_activity_user_id', 'suspicious_activities', ['user_id'])
    op.create_index('idx_suspicious_activity_transaction_id', 'suspicious_activities', ['transaction_id'])
    op.create_index('idx_suspicious_activity_type', 'suspicious_activities', ['activity_type'])
    op.create_index('idx_suspicious_activity_severity', 'suspicious_activities', ['severity'])
    op.create_index('idx_suspicious_activity_status', 'suspicious_activities', ['status'])
    op.create_index('idx_suspicious_activity_detected', 'suspicious_activities', ['detected_at'])
    op.create_index('idx_suspicious_activity_assigned', 'suspicious_activities', ['assigned_to'])
    
    # Create regulatory_reports table
    op.create_table('regulatory_reports',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
        sa.Column('report_type', sa.Enum('sar', 'ctr', 'fbar', 'form_8300', 'suspicious_activity', 'transaction_summary', name='regulatoryreporttype'), nullable=False),
        sa.Column('report_period_start', sa.DateTime(), nullable=False),
        sa.Column('report_period_end', sa.DateTime(), nullable=False),
        sa.Column('status', sa.Enum('draft', 'pending_review', 'approved', 'submitted', 'rejected', name='regulatoryreportstatus'), nullable=False, default='draft'),
    ...
    op.create_index('idx_regulatory_report_period', 'regulatory_reports', ['report_period_start', 'report_period_end'])
    op.create_index('idx_regulatory_report_jurisdiction', 'regulatory_reports', ['jurisdiction'])
    op.create_index('idx_regulatory_report_deadline', 'regulatory_reports', ['submission_deadline'])
    op.create_index('idx_regulatory_report_created', 'regulatory_reports', ['created_at'])
    ...
    op.create_table('risk_assessments',
        ...
        sa.Column('transaction_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('transactions.id'), nullable=True),
        sa.Column('assessment_type', sa.Enum('user_onboarding', 'periodic_review', 'transaction_based', 'portfolio_analysis', 'ad_hoc', name='riskassessmenttype'), nullable=False),
    ...
    op.create_index('idx_risk_assessment_portfolio_id', 'risk_assessments', ['portfolio_id'])
    op.create_index('idx_risk_assessment_transaction_id', 'risk_assessments', ['transaction_id'])
    op.create_index('idx_risk_assessment_type', 'risk_assessments', ['assessment_type'])
    op.create_index('idx_risk_assessment_risk_level', 'risk_assessments', ['risk_level'])
