...
def upgrade():
    ...
    op.create_table('users',
        ...
        sa.Column('password_changed_at', sa.DateTime(), nullable=False, default=sa.func.now()),
        ...
        sa.Column('status', sa.Enum('pending', 'active', 'suspended', 'deactivated', 'banned', name='userstatus'), nullable=False, default='pending'),
    ...
    op.create_index('idx_user_wallet_status', 'users', ['primary_wallet_address', 'status'])
    ...
    op.create_table('user_profiles',
        ...
        sa.Column('risk_tolerance', sa.Enum('low', 'medium', 'high', 'critical', name='risklevel'), nullable=True),
    ...
    op.create_table('user_risk_profiles',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id'), nullable=False, unique=True),
        sa.Column('risk_level', sa.Enum('low', 'medium', 'high', 'critical', name='risklevel'), nullable=False, default='medium'),
    ...
    op.create_index('idx_user_risk_profile_risk_level', 'user_risk_profiles', ['risk_level'])
